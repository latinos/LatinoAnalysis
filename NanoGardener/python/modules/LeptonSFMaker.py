...
class LeptonSFMaker(Module):
    ...
    def initReaders(self,tree): # this function gets the pointers to Value and ArrayReaders and sets them in the C++ worker class
        ...
        for wp in ElectronWP[self.cmssw]['TightObjWP']:
            ...
            for SFkey in ElectronWP[self.cmssw]['TightObjWP'][wp]:
                ...
                if SFkey == 'wpSF':
                    ...
                    for rpr in ElectronWP[self.cmssw]['TightObjWP'][wp]['wpSF']:
                        self.SF_dict['electron'][wp]['wpSF']['beginRP'].append(int(rpr.split('-')[0]))
                        self.SF_dict['electron'][wp]['wpSF']['endRP'].append(int(rpr.split('-')[1]))
                        wp_file = open(cmssw_base + '/src/' + ElectronWP[self.cmssw]['TightObjWP'][wp]['wpSF'][rpr])
        ...
        for wp in MuonWP[self.cmssw]['TightObjWP']:
            ...
            for SFkey in MuonWP[self.cmssw]['TightObjWP'][wp]:
                if SFkey == 'tkSF':
                    self.SF_dict['muon'][wp]['tkSF']['data'] = []
                    self.SF_dict['muon'][wp]['tkSF']['beginRP'] = []
                    self.SF_dict['muon'][wp]['tkSF']['endRP'] = []
                    self.SF_dict['muon'][wp]['tkSF']['SFerror'] = MuonWP[self.cmssw]['TightObjWP'][wp]['tkSFerror']
                    for rpr in MuonWP[self.cmssw]['TightObjWP'][wp]['tkSF']:
                        self.SF_dict['muon'][wp]['tkSF']['beginRP'].append(int(rpr.split('-')[0]))
                        self.SF_dict['muon'][wp]['tkSF']['endRP'].append(int(rpr.split('-')[1]))
                        tk_file = self.open_root(cmssw_base + '/src/' + MuonWP[self.cmssw]['TightObjWP'][wp]['tkSF'][rpr])
                ...
                if SFkey == 'idSF':
                    ...
                    for rpr in MuonWP[self.cmssw]['TightObjWP'][wp]['idSF']:
                        self.SF_dict['muon'][wp]['idSF']['beginRP'].append(int(rpr.split('-')[0]))
                        self.SF_dict['muon'][wp]['idSF']['endRP'].append(int(rpr.split('-')[1]))
                        id_file_data = open(cmssw_base + '/src/' + MuonWP[self.cmssw]['TightObjWP'][wp]['idSF'][rpr][0])
                        id_file_mc = open(cmssw_base + '/src/' + MuonWP[self.cmssw]['TightObjWP'][wp]['idSF'][rpr][1])
                ...
                if SFkey == 'isoSF':
                    ...
                    for rpr in MuonWP[self.cmssw]['TightObjWP'][wp]['isoSF']:
                        self.SF_dict['muon'][wp]['isoSF']['beginRP'].append(int(rpr.split('-')[0]))
                        self.SF_dict['muon'][wp]['isoSF']['endRP'].append(int(rpr.split('-')[1]))
                        id_file_data = open(cmssw_base + '/src/' + MuonWP[self.cmssw]['TightObjWP'][wp]['isoSF'][rpr][0])
                        id_file_mc = open(cmssw_base + '/src/' + MuonWP[self.cmssw]['TightObjWP'][wp]['isoSF'][rpr][1])
