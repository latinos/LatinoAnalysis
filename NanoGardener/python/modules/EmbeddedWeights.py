    ...
    def analyze(self, event):
        ...
        for gen in self.genparts:
          if abs(gen.pdgId)==15:
            ...
            embed_norm *= w.function("m_sel_idEmb_ratio").getValV() # Additional normalization for each individual GenTau
        ...
        if ele_id != -1 and mu_id != -1:
          ...
          if self.year == "2016":
            ...
            else: embed_stitching = 1.0 # Shouldn't happen
          else: # Not needed in 2017
            embed_stitching = 1.0
          ...
          trg_muonelectron_mu23ele12 = getattr(event, 'HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ') # The H -> tautau group that is responsible for embedded samples used only _DZ triggers for mu-e in 2017
          ...
          if (trg_muonelectron_mu8ele23==1) or (trg_muonelectron_mu23ele12==1):
            if embed_hltSF == 0:
        ...
        else: return False # Lepton not reconstructed / couldn't be matched to Tau decay product: Remove event
