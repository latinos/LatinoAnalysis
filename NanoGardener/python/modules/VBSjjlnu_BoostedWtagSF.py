...
class VBSjjlnu_BoostedWtagSF(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        # New Branches
        self.out.branch("VBSjjlnu_BoostedWtagSF_nominal", "F")
        self.out.branch("VBSjjlnu_BoostedWtagSF_up",    "F")
        self.out.branch("VBSjjlnu_BoostedWtagSF_down", "F")
    ...
    def analyze(self, event):
        ...
        VBS_category = event.VBS_category
        
        if VBS_category == 0:
            ## Since the boosted category has been selected at least one good fatjet has passed
            # the selections. SF in high purity region is assigned
            ...
            for ifj, fj in enumerate(fatjets_coll):
                ...
                # Now tau21 check
                if goodFatJet and fj_tau21 > self.max_tau21:
                    # Ok we have found a fatjet passing kinematical cuts but not tau21 --> low purity SF region
                    ...
                    # This should never happen because if you have one good fatjet the event falls in the 
                    # boosted category (and if there are not at least other 2 ak4 jets in the -1 category)
                    # The resolved category requires NO good fatjet with correct tau21 cut. 
                    print '>>>>>>ERROR!'

        # Write out 
        self.out.fillBranch("VBSjjlnu_BoostedWtagSF_nominal", output_SF[0])
        self.out.fillBranch("VBSjjlnu_BoostedWtagSF_up",      output_SF[1])  
        self.out.fillBranch("VBSjjlnu_BoostedWtagSF_down",    output_SF[2])
