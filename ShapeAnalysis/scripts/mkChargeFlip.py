        ...
        for ibinY in range(1, h_dummy.GetNbinsY()+1):
            ...
            for num , ibn in dcsv.items():
                if eta >= float(ibn['etaDown']) and eta < float(ibn['etaUp']) and pt >= float(ibn['ptDown']) and pt < float(ibn['ptUp']):
                    ...
                    data_flip          = float(ibn['DATA']) ; 
                    # data_flip_sys_alt  = float(ibn['DATAsysALT']) ; 
                    # data_flip_sys_mass = float(ibn['DATAsysMASS']); 

                    data_flip_sys = np.sqrt( float(ibn['DATAerr'])*float(ibn['DATAerr']) +
                                             float(ibn['DATAsysALT'])*float(ibn['DATAsysALT']) +
                                             float(ibn['DATAsysMASS'])*float(ibn['DATAsysMASS']) )
...
def outFormat(ifile_, pt_bin_, fitted_prob_, out_, outLO_, output_dir, useCsv=True):
    ...
    for i, ieta in enumerate(eta_bin_array):
        ...
        row += '{:.1f} , {:.1f} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e} , {:.3e}, {:.3e} , {:.3e}, {:.3e}'\
                .format(pt_lo, pt_hi, 
                        fitted_prob_['DATA'][i][0],  fitted_prob_['DATA'][i][1], 
                        abs(float(fitted_prob_['DATA'][i][0]) - float(fitted_prob_['DATA_ALT'][i][0])),
                        abs(float(fitted_prob_['DATA'][i][0]) - float(fitted_prob_['DATA_SMALL_MASS'][i][0])),
    ...
    for i, line in enumerate(row_list):
        header = ['etaDown', 'etaUp', 'ptDown', 'ptUp', 'DATA', 'DATAerr', 'DATAsysALT','DATAsysMASS','MC', 'MCerr', 'MC_LO', 'MC_LOerr', 'SF', 'SFerr', 'SF_LO', 'SF_LOerr', 'totalSys']
        if i==0: 
...
def mkSf(ifile_, ptbin_, year, output_dir, var, outcsv_=True ):
     ...
     h_data                = TFile.Open("{0}/Step3_Chflipfit/{1}/ratio_DATA_{1}.root".                        format(output_dir, name), "READ") # Nominal fit
     h_data_alt            = TFile.Open("{0}/Step3_Chflipfit/{1}/ratio_DATA_{1}_alt.root".                    format(output_dir, name), "READ") # Alternative background function
     h_data_nom_small_mass = TFile.Open("{0}/Step3_Chflipfit/{1}/ratio_DATA_{1}_nom_small_mass_interval.root".format(output_dir, name), "READ") # Smaller mass interval
     ...
     fitted_prob[year] = {}
     h4val[year] = {}
     for ids in ['DATA','DATA_ALT','DATA_SMALL_MASS','MC', 'MC_LO']:
          print(ids)
          if ids == 'DATA':
               h4val[year][ids] = flatten2D(h_data.Get('h2_DATA'))
          elif ids == 'DATA_ALT':
               h4val[year][ids] = flatten2D(h_data_alt.Get('h2_DATA'))
          elif ids == 'DATA_SMALL_MASS':
               h4val[year][ids] = flatten2D(h_data_nom_small_mass.Get('h2_DATA'))
          elif ids == 'MC':
          ...
          print('fitted_prob[{}][{}] = {}'.format(year,ids,fitted_prob[year][ids]))
          print('h4val[{}][{}]       = {}'.format(year,ids,h4val[year][ids]))

     # # Data prob systematic associated to bkg function choice
     # data_sys_alt  = abs(float(fitted_prob[year]['DATA'][0]) - float(fitted_prob[year]['DATA_ALT'][0]))
     # # Data prob systematic associated to fitting range
     # data_sys_mass = abs(float(fitted_prob[year]['DATA'][0]) - float(fitted_prob[year]['DATA_SMALL_MASS'][0]))
     # # Total Data syst
     # data_sys = np.sqrt(data_sys_mass*data_sys_mass + data_sys_alt*data_sys_alt)
    ...
    for iptbin in ptbins :
        ...
        for num, isf in enumerate(epsilon[year]):
             fpout.append('q{} data : {:.3e} +/- {:.3e} ; mc : {:.3e} +/- {:.3e} ; SF : {:.3e} +/- {:.3e} ( rel.error : {:.2f} % ) ; SF LO : {:.3e} +/- {:.3e} (sys. unc. on SF : {:.2f} ), total unc. : {:.2f}'\
                          .format( num, 
                                   mischarge[year]['DATA'][num][0],
                                   np.sqrt(mischarge[year]['DATA'][num][1]            * mischarge[year]['DATA'][num][1] +
                                           abs(mischarge[year]['DATA_ALT'][num][0]-mischarge[year]['DATA'][num][0]) * abs(mischarge[year]['DATA_ALT'][num][0]-mischarge[year]['DATA'][num][0]) + 
                                           abs(mischarge[year]['DATA_SMALL_MASS'][num][0]-mischarge[year]['DATA'][num][0]) * abs(mischarge[year]['DATA_SMALL_MASS'][num][0]-mischarge[year]['DATA'][num][0])),
...
# Signal + bkg fit
def fit(filename, ptbin, output, year, var, flavor, nom_or_alt = 'nom'):
    ...
    for ihis in histos_DATA:
        ...
        if nom_or_alt == 'nom':
             ...
             w.factory("RooCMSShape:bkg(mll,alpha[60., 50., 190.],beta[0.05, 0.01, 0.08],gamma[0.1, -2, 2],peak[91.0])")
        ...
        elif nom_or_alt == 'nom_small_mass_interval':
             # Signal function definition
             w.factory("BreitWigner:sig_bw(mll[81.2, 101.2], bwmean[91.1876,89,93], bwgamma[2.4952,2.4,2.6])")
             w.factory("Gaussian:sig_gau(mll, gaumean[0,-100,100], gausigma[2.5,0.1,5])")
             w.factory("FCONV:bxc(mll,sig_bw,sig_gau)")
             # Background function definition
             w.factory("RooCMSShape:bkg(mll,alpha[60., 50., 190.],beta[0.05, 0.01, 0.08],gamma[0.1, -2, 2],peak[91.0])")
             ...
             w.factory("Gaussian:sig_gau(mll, gaumean[0,-100,100], gausigma[2.5,0.1,5])")
             w.factory("FCONV:bxc(mll,sig_bw,sig_gau)")
             # Alternative background function definition
             w.factory("Exponential:bkg(mll,exalpha[-1.,-10,1])")
             # Sum everything to build the model
             w.factory("SUM:model(nsig[{},0,{}]*bxc, nbkg[{},0,{}]*bkg)".format(str(nHalf), str(nEvent), str(nEvent-nHalf), str(nEvent)))

        else:
             raise ValueError("I don't know this model")
        ...
        plot_name = output + '/c_' + ihis_out_name + '.png'
        if nom_or_alt == 'nom_small_mass_interval':
             plot_name = output + '/c_' + ihis_out_name + '_nom_small_mass_interval.png'
        if nom_or_alt == 'alt':
             plot_name = output + '/c_' + ihis_out_name + '_alt.png'
        ...
        if nom_or_alt == 'nom' or nom_or_alt == 'nom_small_mass_interval':
             w.defineSet("nuisParams", "nbkg,bwmean,bwgamma,gaumean,gausigma,alpha,beta,gamma,peak")
        elif nom_or_alt == 'alt':
    ...
    # Prepare output
    fout_name = output + '/Count_' + ptbin + ".root"
    if nom_or_alt == 'alt':
         fout_name = output + '/Count_' + ptbin + "_alt.root"
    if nom_or_alt == 'nom_small_mass_interval':
         fout_name = output + '/Count_' + ptbin + "_nom_small_mass_interval.root"
...
def ratio(filename, data, ptbin, output, nom_or_alt = 'nom'):
    ...
    fout_name = '{}/ratio_{}_{}_mll.root'.format(output, data, ptbin)
    if nom_or_alt == 'alt':
         fout_name = '{}/ratio_{}_{}_mll_alt.root'.format(output, data, ptbin)
    if nom_or_alt == 'nom_small_mass_interval':
         fout_name = '{}/ratio_{}_{}_mll_nom_small_mass_interval.root'.format(output, data, ptbin)
    ...
    c.SaveAs('{}/h_ss_{}_{}_mll_{}.png'.format(output,data,ptbin,nom_or_alt))
    c.Clear()
    h_os.Draw("colz texte")
    c.SaveAs('{}/h_os_{}_{}_mll_{}.png'.format(output,data,ptbin,nom_or_alt))
    c.Clear()
    h_ratio.Draw("colz texte")
    c.SaveAs('{}/h_ratio_{}_{}_mll_{}.png'.format(output,data,ptbin,nom_or_alt))
...
def mkzfit(input_file, output_dir, era, var, flavor):
    ...
    for iptbin in ptbins :
        ...
        fit(input_file, iptbin, output_1, era, var, flavor, nom_or_alt='alt')
        fit(input_file, iptbin, output_1, era, var, flavor, nom_or_alt='nom_small_mass_interval')
        ...
        for idata in ['DATA', 'DY', 'DY_LO']:
            ...
            ratio("{}/{}/Count_{}_alt.root"                    .format(output_1, name, iptbin), idata, iptbin, '{}/{}'.format(output_2, name), nom_or_alt = "alt")
            ratio("{}/{}/Count_{}_nom_small_mass_interval.root".format(output_1, name, iptbin), idata, iptbin, '{}/{}'.format(output_2, name), nom_or_alt = "nom_small_mass_interval")
